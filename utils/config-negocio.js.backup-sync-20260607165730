...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '9ead31e9-f7c7-40ba-8ce5-c4598fe188a7'; // ID de Norkykm_Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Norkykm_Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '54517017';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'norkykmmayea97@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Norkykm_Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'norkykmnails';
};
...
console.log('✅ config-negocio.js listo para Norkykm_Nails');
